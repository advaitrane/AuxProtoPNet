...
class AuxPPNet(nn.Module):
    ...
    def __init__(
        ...
        """
        Arg(s):
        prototype_class_identity(np.ndarray): A 0-1 array indicating whether a
            prototype is associated with a particular class.
        patch_encoder(str): whether the patch encoder is a separte network or
            the same network as the image encoder. Takes values "same" or 
            "diff"
        """

        super(AuxPPNet, self).__init__()
        ...
        self.update_grads_for_prototypes = update_grads_for_prototypes
        ...
        # We add domain knowledge in the AuxProtoPNet with auxiliary patch 
        # datasets. We pass the prototype class information as input
        ...
        if patch_encoder_type == "diff":
            # Patch encoder follows the same structure
        ...
        if add_on_layers_type == 'bottleneck':
            ...
            if patch_encoder_type == "diff":
                # Patch encoder follows the same structure
                self.patch_add_on_layers = nn.Sequential(*add_on_layers)
        else:
            ...
            if patch_encoder_type == "diff":
                # Patch encoder follows the same structure
                self.patch_add_on_layers = nn.Sequential(
                    nn.Conv2d(in_channels=first_add_on_layer_in_channels, out_channels=self.prototype_shape[1], kernel_size=1),
                    nn.ReLU(),
                    nn.Conv2d(in_channels=self.prototype_shape[1], out_channels=self.prototype_shape[1], kernel_size=1),
                    nn.Sigmoid()
    ...
    def update_prototypes(self, patch_x, patch_labels):
        ...
        if self.patch_encoder_type == "same":
            if self.update_grads_for_prototypes:
                ...
                with torch.no_grad():
                    patch_emb = self.conv_features(patch_x)
        elif self.patch_encoder_type == "diff":
            patch_emb = self.patch_features(patch_x)
            patch_emb = self.patch_add_on_layers(patch_emb)
        else:
            raise RuntimeError(
                "Patch encoder type {} unknown".format(self.patch_encoder_type)
                )

        for idx_emb, emb in enumerate(patch_emb):
